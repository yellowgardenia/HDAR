...
class Anti_Folding():
    ...
    def _diffs(self, y):
        ...
        df = [None] * ndims; weights = [None]*ndims; weighted_df = [None]*ndims
        for i in range(ndims):
            d = i + 1
            # permute dimensions to put the ith dimension first
            r = [d, *range(d), *range(d + 1, ndims + 2)]
            ...
            #calculate the weights for dfi
            zero_ = tf.zeros_like(dfi)
            idxs = tf.cast(tf.where(dfi < zero_,x = dfi, y = zero_),tf.float32)
            weight_dfi = 5*(tf.multiply(tf.abs(dfi),idxs))
            ...
            r = [*range(1, d + 1), 0, *range(d + 1, ndims + 2)]
            # df[i] = K.permute_dimensions(dfi, r)
            # weights[i] = K.permute_dimensions(weight_dfi, r)
            weighted_df[i] = tf.multiply(K.permute_dimensions(dfi, r),K.permute_dimensions(weight_dfi, r))+K.permute_dimensions(dfi, r)
